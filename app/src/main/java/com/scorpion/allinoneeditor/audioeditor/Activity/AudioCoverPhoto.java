...
public class AudioCoverPhoto extends BaseActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(R.layout.audiocoverphoto);
        creationDialog = new ShowCreationDialog(AudioCoverPhoto.this);
        ...
        myHandler.postDelayed(UpdateSongTime,100);
         ...
         Utils.coverimg=getAlbumart(AudioCoverPhoto.this, Utils.albumid);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.done:
//                FBInterstitial.getInstance().displayFBInterstitial(AudioCoverPhoto.this, new FBInterstitial.FbCallback() {
//                    public void callbackCall() {

                ShowInter(AudioCoverPhoto.this);
//                    }
//                });
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PICK_IMAGE && resultCode == RESULT_OK
            ...
            pathh=getPath(selectedImage);
            bitmap = BitmapFactory.decodeFile(pathh);
...
//            cropImage(selectedImage);
    ...
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        public Void doInBackground(Void... voidArr) {
            AudioCoverPhoto videoConverter = AudioCoverPhoto.this;
    ...
    public void make(String str2) {
      ...
      final   String name="/Audio_" + format + ".mp3";
        File file = new File(externalStorageDirectory, getString(R.string.app_name) + "/" + getString(R.string.audiocoverphoto));
        if (!file.exists()) {
            file.mkdir();
        }

        dstPath = file.getAbsolutePath()+name;
//        $ ffmpeg -loop 1 -i image.jpg -i Bryan\ Adams\ -\ Heaven.mp3 -c:v libx264 -c:a aac -strict
//        experimental -b:a 192k -shortest output.mp4
//        ffmpeg -i input.mp3 -i cover.png -c copy -map 0 -map 1 -metadata:s:v
//        title="Album cover" -metadata:s:v comment="Cover (Front)" out.mp3
        String[] strArr = {"-y","-i",str2, "-i", pathh, "-c", "copy","-map","0", "-map",
                "1","-metadata:s:v","title=Album cover","-metadata:s:v","comment=Cover (Front)",dstPath};
        ...
        if (rc == Config.RETURN_CODE_SUCCESS){
            ...
            coverimg=bitmap;
            Intent intent = new Intent(AudioCoverPhoto.this, outputAudioPlayer.class);
